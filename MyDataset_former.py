...
dataset_pathCmp='/home/dream/study/codes/PCCompletion/datasets/dataFromPFNet/shapenet_part/shapenet_part/completepc/'

plt.figure()
class MyDataset(data.Dataset):
    def __init__(self, root=dataset_pathT, three=1, classification=False, class_choice=None, split='train',four_data=0,cmpfolding=0):
        ...
        for item in self.cat:  # item is name eg. lamp
            ...
            dir_cmp_pt = os.path.join(dataset_pathCmp, self.cat[item]) #
                    ...
                    for view in image_view:
                        self.meta[item].append(
                            (os.path.join(dir_point, 'incomplete' + token + '.pts'),
                             ...
                             self.cat[item],
                             token,
                             os.path.join(dir_cmp_pt, 'cmpgt' + token + '.pts')))
                else:
        ...
        for item in self.cat:
            for fn in self.meta[item]:
                self.datapath.append((item, fn[0], fn[1], fn[2], fn[3],fn[4],fn[5]))
    ...
    def __getitem__(self, index):
        if index in self.cache:
            ...
            incomplete, gt, image, foldername, filename,cmpgt= self.cache[index]
        else:
            ...
            cmpgt = np.loadtxt(fn[6]).astype(np.float32)

            foldername = fn[4]
            filename = fn[5]

            if len(self.cache) < self.cache_size:
                self.cache[index] = (incomplete, gt, image, foldername, filename,cmpgt)
        ...
        if self.split=='test':
            ...
            if self.classification:
                return incomplete, gt, image, filename,cmpgt,view_id,center_id
            else:
                return incomplete, gt, image, filename,cmpgt,view_id,center_id
        else:
            if self.classification:
                return incomplete, gt, image, filename,cmpgt
            else:
                return incomplete, gt, image, filename,cmpgt
